...
static int process_event_esp_bootup(struct esp_adapter *adapter, u8 *evt_buf, u8 len)
{
	...
	if (adapter->if_ops && adapter->if_ops->tx_reset)
		adapter->if_ops->tx_reset(adapter);
	...
	if (esp_add_card(adapter)) {
		...
		if (adapter->if_ops && adapter->if_ops->disable_data_path)
			adapter->if_ops->disable_data_path(adapter);
